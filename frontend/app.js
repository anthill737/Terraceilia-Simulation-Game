...
function renderSetup(s){$('title').value=s.title;$('world_text').value=s.world_text;$('players').value=s.players;$('max_days').value=s.max_days;$('max_minutes').value=s.max_minutes||'';$('repo').value=s.repo;$('drama').value=s.drama??6;$('dramaNote').textContent=($('drama').value)+' · '+DRAMA[+$('drama').value];
 ...
 $('vers').innerHTML=Object.entries(s.providers).map(([k,p])=>`${esc(k)}: <b style="color:var(--text)">${p.installed?esc(p.version||'installed'):'not installed'}</b>`).join(' &middot; ')}
...
function openSettings(){if(!S)return;$('g_title').value=S.title||'';$('g_world').value=S.world_text||'';$('g_days').value=S.max_days;$('g_mins').value=S.max_minutes||'';$('g_drama').value=S.drama??6;$('g_dramaNote').textContent=$('g_drama').value+' · '+DRAMA[+$('g_drama').value];$('g_drama').oninput=()=>{$('g_dramaNote').textContent=$('g_drama').value+' · '+DRAMA[+$('g_drama').value]};modelPick('g',S.world_model,$('g_wp'),$('g_wm'),$('g_wc'));renderPeople(S);$('settings').classList.add('open')}
...
document.querySelectorAll('.stabs button').forEach(b=>b.onclick=()=>{sTab=b.dataset.s;document.querySelectorAll('.stabs button').forEach(x=>x.classList.toggle('on',x===b));$('sGame').style.display=sTab==='game'?'':'none';$('sPeople').style.display=sTab==='people'?'':'none'});
