...
function render(s){const first=!S||S.id!==s.id;if(first){T=[];lastTurn=-1}mergeTranscript(s);S=s;providers=s.providers;
 ...
 const msgHtml=(e,i,arr)=>{const prev=i>0?arr[i-1]:null;const key=part(e);const div=key&&(!prev||part(prev)!==key)?`<div class="daypart">${esc(key)}</div>`:'';return div+`<div class="msg ${e.kind}${/^\s*WHISPER\s+@/im.test(e.text)?' whisper':''}${e.kind==='convener'&&/@[A-Za-z]/.test(e.text)?' private':''}" style="${e.color?'--c:'+esc(e.color):''}"><div class="hd"><span class="who">${esc(e.speaker)}</span><span class="meta">Day ${e.day??0}, turn ${e.turn}${e.place?' \u00b7 '+esc(e.place):''}</span></div><div class="body">${rich(e.text)}</div>${e.kind==='speech'&&Array.isArray(e.heard)?`<div class="heard">heard by ${e.heard.length?esc(e.heard.join(', ')):'nobody'}</div>`:''}</div>`};
...
function peoRenderPane(s){const c=s.characters.find(x=>x.name===peoSel);if(!c)return;
 ...
 $('peoHead').innerHTML=`<span class="nm">${esc(c.name)}</span>${title?`, <i class="ttl">${esc(title)}</i>`:''}${showTrade?`, ${esc(/^the /i.test(trade)?trade:'the '+trade)}`:''} \u00b7 ${esc(c.location)} \u00b7 ${c.alive?(c.gone?'gone':'alive'):'dead, '+esc(c.cause_of_death||'unknown')}${(()=>{const g=c.goal||{};const wp=((s.want_progress||{})[c.name])||[0,''];return g.text?` \u00b7 wants ${esc(g.text)}, ${wp[0]}%`:''})()}`;
