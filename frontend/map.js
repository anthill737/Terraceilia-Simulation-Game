...
const MapView=(()=>{
 ...
 const ICONS={
  ...
  'Market town':(g)=>{hut(g,-22,0,14,'#8a6a44');hut(g,4,-6,16,'#9a7a50');hut(g,26,4,12,'#7e6040');el('circle',{cx:-30,cy:18,r:5,fill:'#3a4a55',stroke:'#1a2228'},g);el('rect',{x:-32,y:8,width:4,height:10,fill:'#5a4a30'},g);smoke(g,4,-22);smoke(g,-22,-14)},
  'The inn':(g)=>{el('path',{d:'M-30 18 L-30 -6 L0 -26 L30 -6 L30 18 Z',fill:'#6b4a2e',stroke:'#2a1d10'},g);el('path',{d:'M-33 -4 L0 -28 L33 -4',fill:'none',stroke:'#4a3520','stroke-width':3},g);el('rect',{x:-7,y:2,width:14,height:16,fill:'#1a120a'},g);[-20,14].forEach(x=>el('rect',{x,y:-2,width:8,height:8,fill:'#ffd27a',class:'win'},g));
  ...
  'The forest road':(g)=>{el('path',{d:'M-36 18 Q0 4 36 18',stroke:'#6b5a3e','stroke-width':6,fill:'none'},g);tree(g,-22,-10,8,'#2f6b3d','#1c4128');tree(g,0,-16,10,'#26593a','#1c4128');tree(g,22,-8,8,'#2f6b3d','#1c4128');el('rect',{x:-6,y:4,width:12,height:9,fill:'#3a2a18'},g)},
  'Open country':(g)=>{[-26,-12,2,16,30].forEach(x=>el('path',{d:`M${x} 16 Q${x+3} -6 ${x+6} 16`,stroke:'#c9b25a','stroke-width':3.5,fill:'none'},g));el('path',{d:'M-34 18 L34 18',stroke:'#7a6a3a','stroke-width':2},g);el('path',{d:'M20 -4 L34 -10',stroke:'#7a6a3a','stroke-width':2},g)},
 };
 ...
 function buildBase(s){const svg=$('mapSvg');svg.innerHTML='';const m=s.map;defs(svg);terrain(svg,m);
  ...
  for(const [n,d] of Object.entries(m))for(const a of d.adj){const k=[n,a].sort().join('|');if(seen.has(k)||!m[a])continue;seen.add(k);const unsafe=(n==='The forest road'||a==='The forest road');
  ...
  for(const [n,d] of Object.entries(m)){const node=el('g',{class:'node','data-place':n,transform:`translate(${d.x},${d.y})`},gn);
   el('circle',{r:46,class:'glow'},node);el('ellipse',{cx:0,cy:24,rx:38,ry:9,fill:'#000',opacity:.25},node);const ic=el('g',{class:'icon',filter:'url(#shadow)',transform:'scale(1.15)'},node);(ICONS[n]||ICONS['Ashford'])(ic);
   el('g',{class:'fx'},node);
  ...
  el('text',{x:28,y:38,class:'title',filter:'url(#halo)'},fr).textContent='Terraceilia';el('text',{x:28,y:56,class:'sub',filter:'url(#halo)',id:'dayText'},fr).textContent='';
 ...
 function render(s){S_=s;if(!s.created||!s.map)return;const svg=$('mapSvg');if(!built||svg.dataset.gid!==s.id){buildBase(s);svg.dataset.gid=s.id}
